    ...
    if (
      ...
      await asyncForEach([...trackList].reverse(), async (item: { name: string; item: item }) => {
        if (item.item && item.item.time) {
          ...
          const dataUrl = await domtoimage.toPng(toRender, {
            width: rect.width,
            height: rect.height,
            style: {
              marginLeft: item.item.itemType !== 'text' ? '-20px' : '0',
              marginTop: item.item.itemType !== 'text' ? '-20px' : '0',
              width: rect.width,
              transformOrigin: 'center',
              transform: `translate(0) scale(${rect.width / size[0]}, ${
                rect.height / size[1]
              }) rotate(${rotation}deg)`,
            },
          });
