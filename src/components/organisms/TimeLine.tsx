...
const TimeLine: FC = () => {
    ...
    const calculateItemWidths = () => {
      trackList.forEach((item) => {
        if (item.item && timelineRef.current && item.item.time) {
          const width =
            typeof item.item.width === 'string'
              ? item.item.width.replace('px', '')
              : item.item.width;
          const widthNumber = Number(width) as any;
          const widthDifference = timelineWidth - timelineRef.current.offsetWidth;
          const newWidth =
            widthDifference > 0 ? widthNumber.toFixed() - widthDifference : widthNumber;
          const startTime =
            (item.item.xPosition * videoData.videoLength) / timelineRef.current.offsetWidth;
          const endTime =
            (newWidth * videoData.videoLength) / timelineRef.current.offsetWidth + startTime;
          let newXPosition;
  ...
  const handlePosition = (x: number, _: any) => {
    if (timelineRef.current) {
  ...
  return (
    <Container>
      {dropdownData.name && (
        <SettingsDropdown
          onItemEditClick={handleItemEdit}
          onChangeItemColorClick={handleItemColor}
          onMoveClick={handleMove}
          onDeleteItemClick={handleItemDelete}
          onChangeNameClick={handleNameClick}
          {...dropdownData}
        />
      )}
      <SideNav>
              ...
              return (
                <TrackEdit
                  onSettingsClick={(e: MouseEvent) =>
                    handleSettingsClick(e, item.item?.type, item.name, index)
                  }
                  onDeleteClick={() => handleDeleteTrack(item.name)}
                  key={item.name}
                  name={item.name}
                />
              );
