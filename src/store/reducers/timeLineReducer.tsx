...
export default (state = initState, action: Action): timeLineState => {
  switch (action.type) {
    ...
    case types.MOVE_TRACK: {
      return produce(state, (draft) => {
        const selectedTimeline = draft.timeline.splice(action.payload.index, 1)[0];

        if (action.payload.type === 'up' && action.payload.index !== 0) {
    ...
    case types.UPDATE_ITEM_RATIO: {
      return produce(state, (draft) => {
        draft.timeline.forEach((item) => {
          if (item.name === action.payload.name) {
            if (item.item) {
              item.item.keepRatio = action.payload.keepRatio;
            }
          }
        });
      });
