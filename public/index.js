...
function geocodesToVector3(city) {
  ...
  const x = Math.cos((lat * Math.PI) / 180) * Math.cos((lon * Math.PI) / 180) * r;
  const y = Math.cos((lat * Math.PI) / 180) * Math.sin((lon * Math.PI) / 180) * r;
...
function createSplineDots(origin, destination) {
  ...
  const vertexCountDesired = Math.floor(distanceBetweenCountryCenter * 0.3 + 6) * 2;
  ...
  for (let i = 0; i < numParticles; i++) {
    ...
    const rIndex = constrain(Math.floor(desiredIndex), 0, animationPoints.length - 1);
